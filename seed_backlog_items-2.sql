...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'EMP-DIR' AND bi_title = N'Employee Directory Zone Export')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Zone is not making it to the .csv
...
37148',
        ...
        bi_history = CASE
            WHEN ISNULL(bi_detail_markdown, N'') <> N'Zone is not making it to the .csv
...
37148' THEN
                CASE
                    WHEN bi_history IS NULL OR LTRIM(RTRIM(bi_history)) = N'' THEN N'## 2026-03-09 20:14 UTC - Detail Refresh
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
                    ELSE bi_history + N'

' + N'## 2026-03-09 20:14 UTC - Detail Refresh
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
                END
            ELSE bi_history
        END
    WHERE bi_code = N'EMP-DIR'
      AND bi_title = N'Employee Directory Zone Export'
      AND ISNULL(bi_detail_markdown, N'') <> N'Zone is not making it to the .csv
...
BEGIN
    ...
    (
        ...
        NULL,
        N'Zone is not making it to the .csv
...
Portland,   TN
37148',
        N'## 2026-03-09 20:14 UTC - Seed Import
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'811-TRACK' AND bi_title = N'811 Trackable Reporting Field')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'I need to add something to tickets that is when 811 is required.  If this is checked,  I would want to be able to report on it (with a date, maybe other information).
...
37148',
        bi_lastupdated = GETUTCDATE(),
        bi_history = CASE
            WHEN ISNULL(bi_detail_markdown, N'') <> N'I need to add something to tickets that is when 811 is required.  If this is checked,  I would want to be able to report on it (with a date, maybe other information).
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
            ...
            ELSE bi_history
        END
    WHERE bi_code = N'811-TRACK'
      AND bi_title = N'811 Trackable Reporting Field'
      AND ISNULL(bi_detail_markdown, N'') <> N'I need to add something to tickets that is when 811 is required.  If this is checked,  I would want to be able to report on it (with a date, maybe other information).
...
BEGIN
    ...
    (
        ...
        NULL,
        N'I need to add something to tickets that is when 811 is required.  If this is checked,  I would want to be able to report on it (with a date, maybe other information).
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'CONNECT' AND bi_title = N'Service Item Tab Order')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'A new post has been added to the message board
...
https://www.evotrakker.com/evotech/board?group=10',
        bi_lastupdated = GETUTCDATE(),
        bi_history = CASE
            WHEN ISNULL(bi_detail_markdown, N'') <> N'A new post has been added to the message board
Adding service items for incurred or quoted

When adding a new service items can the tab always populate at the top instead of the bottom?  If you are adding 20+ items we have to scroll to the bottom each time we enter an item.  This time consuming.

View this post on the message board:
https://www.evotrakker.com/evotech/board?group=10' THEN
                CASE
                    WHEN bi_history IS NULL OR LTRIM(RTRIM(bi_history)) = N'' THEN N'## 2026-03-09 20:14 UTC - Detail Refresh
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
                    ELSE bi_history + N'
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
            ...
            ELSE bi_history
        END
    WHERE bi_code = N'CONNECT'
      AND bi_title = N'Service Item Tab Order'
      AND ISNULL(bi_detail_markdown, N'') <> N'A new post has been added to the message board
Adding service items for incurred or quoted
...
BEGIN
    ...
    (
        ...
        NULL,
        N'A new post has been added to the message board
Adding service items for incurred or quoted
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'CONNECT' AND bi_title = N'Swap Quoted / Incurred Items')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'A new post has been added to the message board
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
        ...
        END
    WHERE bi_code = N'CONNECT'
      AND bi_title = N'Swap Quoted / Incurred Items'
      AND ISNULL(bi_detail_markdown, N'') <> N'A new post has been added to the message board
Quoting additions
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'CONNECT' AND bi_title = N'Carry Over WO Attachments')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'A new post has been added to the message board
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
        ...
        END
    WHERE bi_code = N'CONNECT'
      AND bi_title = N'Carry Over WO Attachments'
      AND ISNULL(bi_detail_markdown, N'') <> N'A new post has been added to the message board
Previous Work Order Attachments
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'MTG-0225' AND bi_title = N'811 Utility Marking Checkbox')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Meeting Notes â€” February 25, 2026
...
- €¢ Checkbox â€” primarily tech to check it

- €¢ Capture date checked?

- €¢ Any other info (note)?

- €¢ Report â€” SR #, Date checked, link to SR, other info?

- €¢ New section under PAI

Receipt Report

- €¢ Add drop down similar to WO report â€” default to 1 year, make initial load fast

Verizon Connect Replacement

- €¢ Telemadix / no hardware

- €¢ Geotab â€” API same as what we have with Reveal? (research)

- €¢ Longer term, after high priority items

High Priority Items High Priority

NTE â€” Notification to Tech Phone

- €¢ Calc time / service items, etc.

- €¢ If X% under â€” text tech (90%)

- €¢ If X% over â€” text tech, zone email (100%)

- €¢ Phone numbers needed for all techs

- €¢ Text Message â€” techs
...
- €¢ After SR is created, before Invoice created â€” can we allow changing the Call Center / Company? What issues will that cause?

- €¢ Service Request Quotes â€” able to active/inactive Service Items

- €¢ If inactive, then doesn''t appear in the quote

- €¢ Possible Report â€” drive times, average, by time period, need example',
        bi_lastupdated = GETUTCDATE(),
        bi_history = CASE
            WHEN ISNULL(bi_detail_markdown, N'') <> N'Meeting Notes â€” February 25, 2026
...
- €¢ Possible Report â€” drive times, average, by time period, need example' THEN
                CASE
                    WHEN bi_history IS NULL OR LTRIM(RTRIM(bi_history)) = N'' THEN N'## 2026-03-09 20:14 UTC - Detail Refresh
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
                    ELSE bi_history + N'
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
            ...
            ELSE bi_history
        END
    WHERE bi_code = N'MTG-0225'
      AND bi_title = N'811 Utility Marking Checkbox'
      AND ISNULL(bi_detail_markdown, N'') <> N'Meeting Notes â€” February 25, 2026
...
BEGIN
    ...
    (
        ...
        NULL,
        N'Meeting Notes â€” February 25, 2026
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'MTG-0225' AND bi_title = N'Receipt Report Date Filter')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Meeting Notes â€” February 25, 2026
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
        ...
        END
    WHERE bi_code = N'MTG-0225'
      AND bi_title = N'Receipt Report Date Filter'
      AND ISNULL(bi_detail_markdown, N'') <> N'Meeting Notes â€” February 25, 2026
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'MTG-0225' AND bi_title = N'NTE Utilization Notifications')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Meeting Notes â€” February 25, 2026
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
        ...
        END
    WHERE bi_code = N'MTG-0225'
      AND bi_title = N'NTE Utilization Notifications'
      AND ISNULL(bi_detail_markdown, N'') <> N'Meeting Notes â€” February 25, 2026
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'MTG-0225' AND bi_title = N'New Service Request Flow')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Meeting Notes â€” February 25, 2026
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
        ...
        END
    WHERE bi_code = N'MTG-0225'
      AND bi_title = N'New Service Request Flow'
      AND ISNULL(bi_detail_markdown, N'') <> N'Meeting Notes â€” February 25, 2026
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'MTG-0225' AND bi_title = N'Auto Tech Assignment Logic')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Meeting Notes â€” February 25, 2026
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
        ...
        END
    WHERE bi_code = N'MTG-0225'
      AND bi_title = N'Auto Tech Assignment Logic'
      AND ISNULL(bi_detail_markdown, N'') <> N'Meeting Notes â€” February 25, 2026
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'MTG-0225' AND bi_title = N'Change Call Center Post-Creation')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Meeting Notes â€” February 25, 2026
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
        ...
        END
    WHERE bi_code = N'MTG-0225'
      AND bi_title = N'Change Call Center Post-Creation'
      AND ISNULL(bi_detail_markdown, N'') <> N'Meeting Notes â€” February 25, 2026
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'MTG-0225' AND bi_title = N'SR Quote Service Item Visibility')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Meeting Notes â€” February 25, 2026
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
        ...
        END
    WHERE bi_code = N'MTG-0225'
      AND bi_title = N'SR Quote Service Item Visibility'
      AND ISNULL(bi_detail_markdown, N'') <> N'Meeting Notes â€” February 25, 2026
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'MTG-0225' AND bi_title = N'Drive Time Analytics Report')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Meeting Notes â€” February 25, 2026
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
        ...
        END
    WHERE bi_code = N'MTG-0225'
      AND bi_title = N'Drive Time Analytics Report'
      AND ISNULL(bi_detail_markdown, N'') <> N'Meeting Notes â€” February 25, 2026
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'QB-DASH' AND bi_title = N'QuickBooks Data Import Portal')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Short story  - we are going to need to build more dashboards into EvoTrakker.  We are currently working with an outside consultant (Jim) on what some of those metrics are going to be (more to come on that).  At the same time, I want to integrate an outside
...
37148',
        bi_lastupdated = GETUTCDATE(),
        bi_history = CASE
            WHEN ISNULL(bi_detail_markdown, N'') <> N'Short story  - we are going to need to build more dashboards into EvoTrakker.  We are currently working with an outside consultant (Jim) on what some of those metrics are going to be (more to come on that).  At the same time, I want to integrate an outside
report I currently run (the Profit Report) into EvoTrakker.  The more I talk to Jim, pulling in the data for the Profit Report will probably cover most of the metrics he will suggest, and it accomplishes something I need as well.
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
            ...
            ELSE bi_history
        END
    WHERE bi_code = N'QB-DASH'
      AND bi_title = N'QuickBooks Data Import Portal'
      AND ISNULL(bi_detail_markdown, N'') <> N'Short story  - we are going to need to build more dashboards into EvoTrakker.  We are currently working with an outside consultant (Jim) on what some of those metrics are going to be (more to come on that).  At the same time, I want to integrate an outside
report I currently run (the Profit Report) into EvoTrakker.  The more I talk to Jim, pulling in the data for the Profit Report will probably cover most of the metrics he will suggest, and it accomplishes something I need as well.
...
BEGIN
    ...
    (
        ...
        NULL,
        N'Short story  - we are going to need to build more dashboards into EvoTrakker.  We are currently working with an outside consultant (Jim) on what some of those metrics are going to be (more to come on that).  At the same time, I want to integrate an outside
report I currently run (the Profit Report) into EvoTrakker.  The more I talk to Jim, pulling in the data for the Profit Report will probably cover most of the metrics he will suggest, and it accomplishes something I need as well.
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'QB-DASH' AND bi_title = N'Profit Report Lookup Tables')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Short story  - we are going to need to build more dashboards into EvoTrakker.  We are currently working with an outside consultant (Jim) on what some of those metrics are going to be (more to come on that).  At the same time, I want to integrate an outside
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
        ...
        END
    WHERE bi_code = N'QB-DASH'
      AND bi_title = N'Profit Report Lookup Tables'
      AND ISNULL(bi_detail_markdown, N'') <> N'Short story  - we are going to need to build more dashboards into EvoTrakker.  We are currently working with an outside consultant (Jim) on what some of those metrics are going to be (more to come on that).  At the same time, I want to integrate an outside
report I currently run (the Profit Report) into EvoTrakker.  The more I talk to Jim, pulling in the data for the Profit Report will probably cover most of the metrics he will suggest, and it accomplishes something I need as well.
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'QB-DASH' AND bi_title = N'Cost Allocation Methodology')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Short story  - we are going to need to build more dashboards into EvoTrakker.  We are currently working with an outside consultant (Jim) on what some of those metrics are going to be (more to come on that).  At the same time, I want to integrate an outside
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
        ...
        END
    WHERE bi_code = N'QB-DASH'
      AND bi_title = N'Cost Allocation Methodology'
      AND ISNULL(bi_detail_markdown, N'') <> N'Short story  - we are going to need to build more dashboards into EvoTrakker.  We are currently working with an outside consultant (Jim) on what some of those metrics are going to be (more to come on that).  At the same time, I want to integrate an outside
report I currently run (the Profit Report) into EvoTrakker.  The more I talk to Jim, pulling in the data for the Profit Report will probably cover most of the metrics he will suggest, and it accomplishes something I need as well.
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'QB-DASH' AND bi_title = N'Zone & Tech Report Outputs')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Short story  - we are going to need to build more dashboards into EvoTrakker.  We are currently working with an outside consultant (Jim) on what some of those metrics are going to be (more to come on that).  At the same time, I want to integrate an outside
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
        ...
        END
    WHERE bi_code = N'QB-DASH'
      AND bi_title = N'Zone & Tech Report Outputs'
      AND ISNULL(bi_detail_markdown, N'') <> N'Short story  - we are going to need to build more dashboards into EvoTrakker.  We are currently working with an outside consultant (Jim) on what some of those metrics are going to be (more to come on that).  At the same time, I want to integrate an outside
report I currently run (the Profit Report) into EvoTrakker.  The more I talk to Jim, pulling in the data for the Profit Report will probably cover most of the metrics he will suggest, and it accomplishes something I need as well.
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'QB-DASH' AND bi_title = N'Trip Charge Utilization Fix')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Short story  - we are going to need to build more dashboards into EvoTrakker.  We are currently working with an outside consultant (Jim) on what some of those metrics are going to be (more to come on that).  At the same time, I want to integrate an outside
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
        ...
        END
    WHERE bi_code = N'QB-DASH'
      AND bi_title = N'Trip Charge Utilization Fix'
      AND ISNULL(bi_detail_markdown, N'') <> N'Short story  - we are going to need to build more dashboards into EvoTrakker.  We are currently working with an outside consultant (Jim) on what some of those metrics are going to be (more to come on that).  At the same time, I want to integrate an outside
report I currently run (the Profit Report) into EvoTrakker.  The more I talk to Jim, pulling in the data for the Profit Report will probably cover most of the metrics he will suggest, and it accomplishes something I need as well.
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'AZURE-CERT' AND bi_title = N'Validate App Service Domain')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Domain ownership validation is required for certificate issuance and renewal starting 1 March 2026.
...
Microsoft Corporation,  One Microsoft Way, Redmond, WA 98052',
        bi_lastupdated = GETUTCDATE(),
        bi_history = CASE
            WHEN ISNULL(bi_detail_markdown, N'') <> N'Domain ownership validation is required for certificate issuance and renewal starting 1 March 2026.
...
Microsoft Corporation,  One Microsoft Way, Redmond, WA 98052' THEN
                CASE
                    WHEN bi_history IS NULL OR LTRIM(RTRIM(bi_history)) = N'' THEN N'## 2026-03-09 20:14 UTC - Detail Refresh
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
                    ELSE bi_history + N'
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
            ...
            ELSE bi_history
        END
    WHERE bi_code = N'AZURE-CERT'
      AND bi_title = N'Validate App Service Domain'
      AND ISNULL(bi_detail_markdown, N'') <> N'Domain ownership validation is required for certificate issuance and renewal starting 1 March 2026.
...
BEGIN
    ...
    (
        ...
        NULL,
        N'Domain ownership validation is required for certificate issuance and renewal starting 1 March 2026.
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'KEY-VAULT' AND bi_title = N'Azure Key Vault RBAC Migration')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'You probably got this already
...
Microsoft Corporation, One Microsoft Way, Redmond, WA 98052',
        bi_lastupdated = GETUTCDATE(),
        bi_history = CASE
            WHEN ISNULL(bi_detail_markdown, N'') <> N'You probably got this already
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
            ...
            ELSE bi_history
        END
    WHERE bi_code = N'KEY-VAULT'
      AND bi_title = N'Azure Key Vault RBAC Migration'
      AND ISNULL(bi_detail_markdown, N'') <> N'You probably got this already
...
BEGIN
    ...
    (
        ...
        NULL,
        N'You probably got this already
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'GPS-EVAL' AND bi_title = N'Geotab GPS Provider Evaluation')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Chris,
...
37148',
        bi_lastupdated = GETUTCDATE(),
        bi_history = CASE
            WHEN ISNULL(bi_detail_markdown, N'') <> N'Chris,
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
            ...
            ELSE bi_history
        END
    WHERE bi_code = N'GPS-EVAL'
      AND bi_title = N'Geotab GPS Provider Evaluation'
      AND ISNULL(bi_detail_markdown, N'') <> N'Chris,
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'WO-NOTES' AND bi_title = N'WO Notes Field Always Visible')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Daniel,
...
Direct: 615-306-6463',
        bi_lastupdated = GETUTCDATE(),
        bi_history = CASE
            WHEN ISNULL(bi_detail_markdown, N'') <> N'Daniel,
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
            ...
            ELSE bi_history
        END
    WHERE bi_code = N'WO-NOTES'
      AND bi_title = N'WO Notes Field Always Visible'
      AND ISNULL(bi_detail_markdown, N'') <> N'Daniel,
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'PAYROLL' AND bi_title = N'Guaranteed Hours User Field')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Chris,
...
•
Kind of a wish list thing - if in the future they have not hit 32 hours maybe Adjustable payable hours block might show red and if not 40 maybe show yellow. Reason I ask for this is When it move over to excel
I format the cells to show me which ones have an issue - just looking at in the system is easy to miss one' THEN
                CASE
                    WHEN bi_history IS NULL OR LTRIM(RTRIM(bi_history)) = N'' THEN N'## 2026-03-09 20:14 UTC - Detail Refresh
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
                    ELSE bi_history + N'
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
            ...
            ELSE bi_history
        END
    WHERE bi_code = N'PAYROLL'
      AND bi_title = N'Guaranteed Hours User Field'
      AND ISNULL(bi_detail_markdown, N'') <> N'Chris,
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'PAYROLL' AND bi_title = N'Employee Number on Report')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Chris,
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
        ...
        END
    WHERE bi_code = N'PAYROLL'
      AND bi_title = N'Employee Number on Report'
      AND ISNULL(bi_detail_markdown, N'') <> N'Chris,
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'PAYROLL' AND bi_title = N'Worksheet PTO & Leave Adjustments')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Chris,
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
        ...
        END
    WHERE bi_code = N'PAYROLL'
      AND bi_title = N'Worksheet PTO & Leave Adjustments'
      AND ISNULL(bi_detail_markdown, N'') <> N'Chris,
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'PAYROLL' AND bi_title = N'Direct Payroll Export Workflow')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Chris,
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
        ...
        END
    WHERE bi_code = N'PAYROLL'
      AND bi_title = N'Direct Payroll Export Workflow'
      AND ISNULL(bi_detail_markdown, N'') <> N'Chris,
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'CALL-CTR' AND bi_title = N'Non-Admin Call Center Access')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'I also need feeding into this report, those companies (under Direct Commercial for example) info about their portals.  This would require that same type of portal gathering info to be added to companies.
...
37148',
        bi_lastupdated = GETUTCDATE(),
        bi_history = CASE
            WHEN ISNULL(bi_detail_markdown, N'') <> N'I also need feeding into this report, those companies (under Direct Commercial for example) info about their portals.  This would require that same type of portal gathering info to be added to companies.
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
            ...
            ELSE bi_history
        END
    WHERE bi_code = N'CALL-CTR'
      AND bi_title = N'Non-Admin Call Center Access'
      AND ISNULL(bi_detail_markdown, N'') <> N'I also need feeding into this report, those companies (under Direct Commercial for example) info about their portals.  This would require that same type of portal gathering info to be added to companies.
...
BEGIN
    ...
    (
        ...
        NULL,
        N'I also need feeding into this report, those companies (under Direct Commercial for example) info about their portals.  This would require that same type of portal gathering info to be added to companies.
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'CALL-CTR' AND bi_title = N'Call Center Portal Info Report')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'I also need feeding into this report, those companies (under Direct Commercial for example) info about their portals.  This would require that same type of portal gathering info to be added to companies.
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
        ...
        END
    WHERE bi_code = N'CALL-CTR'
      AND bi_title = N'Call Center Portal Info Report'
      AND ISNULL(bi_detail_markdown, N'') <> N'I also need feeding into this report, those companies (under Direct Commercial for example) info about their portals.  This would require that same type of portal gathering info to be added to companies.
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'RECEIPTS' AND bi_title = N'PDF Re-upload Receipt Bug')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Very specific bug related to (I assume) uploading PDF files.
...
37148',
        bi_lastupdated = GETUTCDATE(),
        bi_history = CASE
            WHEN ISNULL(bi_detail_markdown, N'') <> N'Very specific bug related to (I assume) uploading PDF files.
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
            ...
            ELSE bi_history
        END
    WHERE bi_code = N'RECEIPTS'
      AND bi_title = N'PDF Re-upload Receipt Bug'
      AND ISNULL(bi_detail_markdown, N'') <> N'Very specific bug related to (I assume) uploading PDF files.
...
BEGIN
    ...
    (
        ...
        NULL,
        N'Very specific bug related to (I assume) uploading PDF files.
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'LOGOUT' AND bi_title = N'Auto-Save on Session Expiry')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'I spoke with Daniel earlier today after Mike was logged out in the middle of working on a quote. He finished typing his verbiage in and went to save and it immediately logged him out. This was around ~2:30pm. Assuming Mike got here around 6:30am, this would
...
Direct: 615-306-6463',
        bi_lastupdated = GETUTCDATE(),
        bi_history = CASE
            WHEN ISNULL(bi_detail_markdown, N'') <> N'I spoke with Daniel earlier today after Mike was logged out in the middle of working on a quote. He finished typing his verbiage in and went to save and it immediately logged him out. This was around ~2:30pm. Assuming Mike got here around 6:30am, this would
be around the 8 hour mark. Daniel explained this is a new security feature where it previously logged everyone out after 4 hours, after negotiating was changed to 8 hours.
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
            ...
            ELSE bi_history
        END
    WHERE bi_code = N'LOGOUT'
      AND bi_title = N'Auto-Save on Session Expiry'
      AND ISNULL(bi_detail_markdown, N'') <> N'I spoke with Daniel earlier today after Mike was logged out in the middle of working on a quote. He finished typing his verbiage in and went to save and it immediately logged him out. This was around ~2:30pm. Assuming Mike got here around 6:30am, this would
be around the 8 hour mark. Daniel explained this is a new security feature where it previously logged everyone out after 4 hours, after negotiating was changed to 8 hours.
...
BEGIN
    ...
    (
        ...
        NULL,
        N'I spoke with Daniel earlier today after Mike was logged out in the middle of working on a quote. He finished typing his verbiage in and went to save and it immediately logged him out. This was around ~2:30pm. Assuming Mike got here around 6:30am, this would
be around the 8 hour mark. Daniel explained this is a new security feature where it previously logged everyone out after 4 hours, after negotiating was changed to 8 hours.
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'SKILLS' AND bi_title = N'Skill Level Category Renumber')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'I would like to add a category in Skills and change the numbers (push them up and add 5) to allow space for it.
...
37148',
        bi_lastupdated = GETUTCDATE(),
        bi_history = CASE
            WHEN ISNULL(bi_detail_markdown, N'') <> N'I would like to add a category in Skills and change the numbers (push them up and add 5) to allow space for it.
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
            ...
            ELSE bi_history
        END
    WHERE bi_code = N'SKILLS'
      AND bi_title = N'Skill Level Category Renumber'
      AND ISNULL(bi_detail_markdown, N'') <> N'I would like to add a category in Skills and change the numbers (push them up and add 5) to allow space for it.
...
BEGIN
    ...
    (
        ...
        NULL,
        N'I would like to add a category in Skills and change the numbers (push them up and add 5) to allow space for it.
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'EVINES-DEC' AND bi_title = N'Supplemental PTO Request Window')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Hey y''all. In the attached Word doc, I added a little detail to what I talked about on the call yesterday plus added one forward looking Evotrakker request.
...
Eric V',
        bi_lastupdated = GETUTCDATE(),
        bi_history = CASE
            WHEN ISNULL(bi_detail_markdown, N'') <> N'Hey y''all. In the attached Word doc, I added a little detail to what I talked about on the call yesterday plus added one forward looking Evotrakker request.
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
            ...
            ELSE bi_history
        END
    WHERE bi_code = N'EVINES-DEC'
      AND bi_title = N'Supplemental PTO Request Window'
      AND ISNULL(bi_detail_markdown, N'') <> N'Hey y''all. In the attached Word doc, I added a little detail to what I talked about on the call yesterday plus added one forward looking Evotrakker request.
...
BEGIN
    ...
    (
        ...
        NULL,
        N'Hey y''all. In the attached Word doc, I added a little detail to what I talked about on the call yesterday plus added one forward looking Evotrakker request.
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'EVINES-DEC' AND bi_title = N'Drive Time & Distance per Work Order')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Hey y''all. In the attached Word doc, I added a little detail to what I talked about on the call yesterday plus added one forward looking Evotrakker request.
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
        ...
        END
    WHERE bi_code = N'EVINES-DEC'
      AND bi_title = N'Drive Time & Distance per Work Order'
      AND ISNULL(bi_detail_markdown, N'') <> N'Hey y''all. In the attached Word doc, I added a little detail to what I talked about on the call yesterday plus added one forward looking Evotrakker request.
...
IF EXISTS (SELECT 1 FROM dbo.backlogitem WHERE bi_code = N'EVINES-DEC' AND bi_title = N'Quarterly Continuing Education Requirement')
BEGIN
    UPDATE dbo.backlogitem
    SET
        bi_detail_markdown = N'Hey y''all. In the attached Word doc, I added a little detail to what I talked about on the call yesterday plus added one forward looking Evotrakker request.
...
- Refreshed detailMarkdown from backlog reference content and preserved line breaks.'
        ...
        END
    WHERE bi_code = N'EVINES-DEC'
      AND bi_title = N'Quarterly Continuing Education Requirement'
      AND ISNULL(bi_detail_markdown, N'') <> N'Hey y''all. In the attached Word doc, I added a little detail to what I talked about on the call yesterday plus added one forward looking Evotrakker request.
